...
// [BEGIN] Added for pppoe function [Willie][2011-08-15] 
int do_pppoe(const char *user,const char *password);
int stop_pppoe();
char *pppoe_get_ip();
char *pppoe_get_gateway();
char *pppoe_get_dns1();
char *pppoe_get_dns2();
char *pppoe_get_ifname() ;
int ifc_PPPoEconfigure(const char *ifname,in_addr_t dns1,in_addr_t dns2); 
// [END]
}
...
// [BEGIN] run PPPoE mode [Willie][2011-06-30]
static jint android_net_utils_runPPPoE(JNIEnv* env, jobject clazz, jstring name, jstring password)
{
	jboolean isCopy;
	int result;
	const char *nameStr = env->GetStringUTFChars(name, &isCopy);
	const char *passwordStr = env->GetStringUTFChars(password, &isCopy);
	result = ::do_pppoe(nameStr, passwordStr);
	env->ReleaseStringUTFChars(name, nameStr); 
	env->ReleaseStringUTFChars(password, passwordStr);
	return (jint)result;
}

static jint android_net_utils_stopPPPoE(JNIEnv* env, jobject clazz)
{
    int result;
    result = ::stop_pppoe();
    return (jboolean)(result == 0);
...
static jstring android_net_utils_getPppoeIp(JNIEnv* env, jobject clazz)
{
    return env->NewStringUTF(::pppoe_get_ip());
}

static jstring android_net_utils_getPppoeGateway(JNIEnv* env, jobject clazz)
{
    return env->NewStringUTF(::pppoe_get_gateway());
}

static jstring android_net_utils_getPppoeDns1(JNIEnv* env, jobject clazz)
{
    return env->NewStringUTF(::pppoe_get_dns1());
}

static jstring android_net_utils_getPppoeDns2(JNIEnv* env, jobject clazz)
{
    return env->NewStringUTF(::pppoe_get_dns2());
}

static jstring android_net_utils_getPppoeIfname(JNIEnv* env, jobject clazz)
{
    return env->NewStringUTF(::pppoe_get_ifname());
}

static jboolean android_net_utils_PPPoEconfigureNative(JNIEnv* env,
        jobject clazz,
        jstring ifname,
        jint dns1,
        jint dns2)
{
    ...
    const char *nameStr = env->GetStringUTFChars(ifname, NULL);
    result = ::ifc_PPPoEconfigure(nameStr, dns1, dns2);
    env->ReleaseStringUTFChars(ifname, nameStr);
    return (jboolean)(result == 0);
}
...
static JNINativeMethod gNetworkUtilMethods[] = {
    ...
    // [BEGIN] set PPPoE ethernet mode [willie] [2011-06-30] 
    { "runPPPoE", "(Ljava/lang/String;Ljava/lang/String;)I", (void*) android_net_utils_runPPPoE },
    { "stopPPPoE", "()Z", (void*) android_net_utils_stopPPPoE },
    { "getPppoeIp", "()Ljava/lang/String;", (void*) android_net_utils_getPppoeIp },
    { "getPppoeGateway", "()Ljava/lang/String;", (void*) android_net_utils_getPppoeGateway },
    { "getPppoeDns1", "()Ljava/lang/String;", (void*) android_net_utils_getPppoeDns1},
    { "getPppoeDns2", "()Ljava/lang/String;", (void*) android_net_utils_getPppoeDns2 },
    { "getPppoeIfname", "()Ljava/lang/String;", (void*) android_net_utils_getPppoeIfname },
    { "PPPoEconfigureNative", "(Ljava/lang/String;II)Z", (void*) android_net_utils_PPPoEconfigureNative },
    // [END]
};
