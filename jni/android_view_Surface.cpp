...
// [BEGIN] skyviia modify: set display mode
static void Surface_setDisplayMode(
        JNIEnv* env, jobject clazz, jint display, jint mode)
{
    int err = SurfaceComposerClient::setDisplayMode(display, mode);
    if (err < 0) {
        doThrow(env, "java/lang/IllegalArgumentException", NULL);
    }
}

static void Surface_setCursorBmp(
        JNIEnv* env, jobject clazz, jbyteArray bmpCursor, jint size)
{
    jbyte* dataBytes = env->GetByteArrayElements(bmpCursor, NULL);
    if (dataBytes == NULL) {
        doThrow(env, "java/lang/NullPointerException", NULL);
		return;
    }
    int err = SurfaceComposerClient::setCursorBmp((uint8_t *)dataBytes, size);
    if (err < 0) {
        doThrow(env, "java/lang/IllegalArgumentException", NULL);
    }
	env->ReleaseByteArrayElements(bmpCursor, dataBytes, 0);
}

static void Surface_setCursorPos(
        JNIEnv* env, jobject clazz, jint enable, jint x, jint y, jint alpha)
{
    int err = SurfaceComposerClient::setCursorPos(enable, x, y, alpha);
    if (err < 0) {
        doThrow(env, "java/lang/IllegalArgumentException", NULL);
    }
}
...
// [BEGIN] skyviia modify: set Display Mode
	{"setDisplayMode",      "(II)V",(void*)Surface_setDisplayMode },
	{"setCursorBmp",	   "([BI)V", (void*)Surface_setCursorBmp },
	{"setCursorPos",	   "(IIII)V", (void*)Surface_setCursorPos },
// [END]
