...
{
    // [BEGIN] Added for save wifi information to env nand flash [Willie][2011-08-19]
    jboolean result;
    // [END]
    ...
    result = doBooleanCommand("SAVE_CONFIG", "OK");
	return result&&(jboolean)(::save_conf() == 0);
	// [END]
}
...
/* [BEGIN] Willie added, for set wps */
static jboolean android_net_wifi_startWpsPbcCommond(JNIEnv* env, jobject clazz)
{
	LOGW("startWpsPbcCommond");
    return doBooleanCommand("WPS_PBC", "OK");
}

static jboolean android_net_wifi_startWpsPinCommond(JNIEnv* env, jobject clazz,jstring pinkey)
{	
	char cmdstr[256];
    jboolean isCopy;

    const char *pinkeyStr = env->GetStringUTFChars(pinkey, &isCopy);

    int cmdTooLong = snprintf(cmdstr, sizeof(cmdstr), "WPS_PIN any %s", pinkeyStr) >= (int)sizeof(cmdstr);

    env->ReleaseStringUTFChars(pinkey, pinkeyStr);

    return (jboolean)!cmdTooLong && doBooleanCommand(cmdstr, "OK");
}

static jboolean android_net_wifi_stopWpsCommond(JNIEnv* env, jobject clazz)
{
    //return doBooleanCommand("BLACKLIST clear", "OK");
	return doBooleanCommand("WPS_STOP", "OK");
}


/* [END] */

// [BEGIN] Add save to env nand flash data [Willie][2011-08-25]
static jboolean android_net_wifi_updateEnvConfig(JNIEnv* env, jobject clazz)
{
   return (jboolean)(::save_conf() == 0);
}
// [END]
...
static JNINativeMethod gWifiMethods[] = {
    ...
    /* [BEGIN] Willie added , for wps set */
	{ "startWpsPbcCommond", "()Z", (void*) android_net_wifi_startWpsPbcCommond },
	{ "startWpsPinCommond", "(Ljava/lang/String;)Z", (void*) android_net_wifi_startWpsPinCommond },
    { "stopWpsCommond", "()Z", (void*) android_net_wifi_stopWpsCommond },
	/* [END] */
	// [BEGIN] Add save to env nand flash data [Willie][2011-08-25]
	{ "updateEnvConfig", "()Z", (void*) android_net_wifi_updateEnvConfig },
	// [END]
};
