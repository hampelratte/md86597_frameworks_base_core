...
// [BEGIN] skyviia modify: [Willie Chen] added for zhuyin input methods
...
public class KeyboardView extends View implements View.OnClickListener {
    ...
    // [BEGIN] skyviia modify: [Willie Chen] add for adding Repeat Key Index from IR
    private int mRepeatKeyIndexFromIR = NOT_A_KEY;
    // [END]
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MSG_SHOW_PREVIEW:
					// [BEGIN] remove show preview function [Willie][2011-05-06]
                    //showKey(msg.arg1);
                    // [END]
                    break;
    ...
    private void showPreview(int keyIndex) {
		...
		// [BEGIN] remove preview function [Willie][2011-05-06]
		/*
    ...
    private boolean repeatKey() {
		// [BEGIN] modify check key limit [Willie][2011-05-19]
        if((mRepeatKeyIndex <= NOT_A_KEY)||
			(mKeys.length <= mRepeatKeyIndex)){
           return false;
		}
		// [END]
    ...
    public boolean handleBack() {
        if (mPopupKeyboard.isShowing()) {
            dismissPopupKeyboard();
			/* [BEGIN] Willie added for send holomorphic word */
			mRepeatKeyIndexFromIR = NOT_A_KEY;
			/* [END] */
            return true;
        }
        return false;
    ...
    // [BEGIN] skyviia modify: [Willie Chen] add for show key focus
    // show the keycode focus from the key index
		public void showFocus(int keyIndex) {
        int oldKeyIndex = mCurrentKeyIndex;
        mCurrentKeyIndex = keyIndex;
        // Release the old key and press the new key
        final Key[] keys = mKeys;
    ...
    // remove the focus
		public void removeFocus(int keyIndex) {
        mHandler.removeMessages(MSG_SHOW_PREVIEW);
        mHandler.removeMessages(MSG_LONGPRESS);
        showPreview(NOT_A_KEY);
        Arrays.fill(mKeyIndices, NOT_A_KEY);
    ...
    // send long press message
		public void sendLongPressMessage(int keyIndex) {
		
        mCurrentKey = keyIndex;
			
        // check repeat key if Curreny key = Repeat Key is repeat key state
        if(mRepeatKeyIndexFromIR == mCurrentKey)
        return;
        ...
        // Start new longpress if key has changed
        if (keyIndex != NOT_A_KEY) {
            mHandler.sendEmptyMessageDelayed(MSG_LONGPRESS, LONGPRESS_TIMEOUT);
        }
			mRepeatKeyIndexFromIR = keyIndex;
    }
    
    // remove long press key state
    public void removeLongPressMessage(){
		...
		// get popup keyboard state if 
		public boolean getMiniKeyboardState() {
			return mMiniKeyboardOnScreen;
    }

		// get popup keyboard
		public KeyboardView getPopupKeyboard() {
