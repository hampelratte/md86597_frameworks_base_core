...
// [BEGIN] skyviia modify: for removing accessibility service!
        AccessibilityManager accmgr = AccessibilityManager.getInstance(mContext);
        if (accmgr != null && accmgr.isEnabled()) {
...
// [BEGIN] skyviia modify: for removing accessibility service!
        ...
        AccessibilityManager accmgr = AccessibilityManager.getInstance(mContext);
        if (accmgr != null) {
            accmgr.sendAccessibilityEvent(event);
        }
// [END]
    }
...
// [BEGIN] skyviia modify: porting for debug key layout mapping & key event!
//        Log.d(VIEW_LOG_TAG, "dispatchKeyEventPreIme() : " + event);
// [END]
