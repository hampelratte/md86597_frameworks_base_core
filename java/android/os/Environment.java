...
//ethan A3
	public static String getSelectedStorageID(Context context) {
		return Settings.System.getString(
			context.getContentResolver(), Settings.System.SELECTED_DEVICE_ID);
	}
// [BEGIN] skyviia modify: [Mark Yang] add for Media Selector! [2010-04-01] change to public by Ethan Chen
	public static String getSelectedStorageEntry(Context context) {
		return Settings.System.getString(
			context.getContentResolver(), Settings.System.SELECTED_DEVICE);
	}
// [BEGIN] add By Minchay_0302 20111028
	public static String getSelectedStorageResult(Context context) {
		return Settings.System.getString(
			context.getContentResolver(), Settings.System.SELECTED_DEVICE_RESULT);
	}

        /**
        * Gets the Android selected storage directory from media selector.
        */
	public static File getSelectedStorageDirectory(Context context) {
		return getStorageDirectory(getSelectedStorageEntry(context));
	}
	
	public static File getStorageDirectory(String path) {
    ...
    /**
     * getSelectedStorageDirectory() returns MEDIA_UNMOUNTABLE if the media is present
     * but cannot be mounted.  Typically this happens if the file system on the
     * media is corrupted. 
     */
    public static String getSelectedStorageState(Context context) {
	    String strFolderName = getSelectedStorageEntry(context);
		return ((strFolderName != null) ? 
			SystemProperties.get(strFolderName, MEDIA_REMOVED) : MEDIA_REMOVED);
	}
	
    public static String getStorageState(String path) {
		return ((path != null) ? 
			SystemProperties.get(path, MEDIA_REMOVED) : MEDIA_REMOVED);
	}

	//[BEGIN] Add for checking selected device (Skyviia-Michael Jan. 20, 2011)
	/**
	 * Check whether the current selected media is UPnP or not.
	 * @param context Context of the caller.
	 * @return true if the selected media is UPnP.
	 */
	public static boolean isUPnPSelected(Context context)
	{
	    String strSelectedMedia = Environment.getSelectedStorageEntry(context);
	    if(null == strSelectedMedia ||
	       0 == strSelectedMedia.length())
	    {
	        return false;
	    }

	    if(strSelectedMedia.toUpperCase().startsWith("DLNA"))
	    {
    	    return true;
    	}
	...
	/**
	 * Check whether the current selected media is samba or not.
	 * @param context Context of the caller.
	 * @return true if the selected media is samba.
	 */
	public static boolean isSambaSelected(Context context)
	{
	    String strSelectedMedia = Environment.getSelectedStorageEntry(context);
	    if(null == strSelectedMedia ||
	       0 == strSelectedMedia.length())
        ...
        ArrayList<MountInfo.MountStorage> entries = null;
        try
        {
            entries = ms.getMountedInfo().getAllEntries();
            MountInfo.MountStorage mnt = null;
            int nEntryCount = entries.size();
            String curType = null;
            for(int nEntryIndex = 0; nEntryIndex < nEntryCount; nEntryIndex++)
            {
                mnt = entries.get(nEntryIndex);
                curType = mnt.getType().toString();
                if(strSelectedMedia.equals(mnt.getPath()) &&
                   curType.equals(MountInfo.TYPES.SAMBA.toString()))
                {
                    return true;
                }
            }
	...
	//[END] Add for checking selected device
// [END]
