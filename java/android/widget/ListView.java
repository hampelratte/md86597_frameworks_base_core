...
public class ListView extends AbsListView {
    ...
    private boolean commonKey(int keyCode, int count, KeyEvent event) {
        ...
        if (action != KeyEvent.ACTION_UP) {
            ...
            case KeyEvent.KEYCODE_MEDIA_REWIND:
                pageScroll(FOCUS_UP, 3);
                break;
            case KeyEvent.KEYCODE_MEDIA_FAST_FORWARD:
                pageScroll(FOCUS_DOWN, 3);
                break;
            case KeyEvent.KEYCODE_MEDIA_PREVIOUS:
                pageScroll(FOCUS_UP, 10);
                break;
            case KeyEvent.KEYCODE_MEDIA_NEXT:
                pageScroll(FOCUS_DOWN, 10);
                break;
            case KeyEvent.KEYCODE_DPAD_CENTER:
            case KeyEvent.KEYCODE_ENTER:
    ...
    boolean pageScroll(int direction) {
        return pageScroll(direction, 1);
    }

    /**
     * Scrolls up or down by the number of items currently present on screen numberOfPages times.
     *
     * @param direction either {@link View#FOCUS_UP} or {@link View#FOCUS_DOWN}
     * @param numberOfPages the number of pages to scroll
     * @return whether selection was moved
     */
    boolean pageScroll(int direction, int numberOfPages) {
        int nextPage = -1;
        boolean down = false;
        ...
        int visibleCells = lastPosition - firstPosition;

        if (direction == FOCUS_UP) {
            // nextPage = Math.max(0, mSelectedPosition - getChildCount() - 1);
            nextPage = Math.max(0, selectedIndex - (visibleCells * numberOfPages));
        } else if (direction == FOCUS_DOWN) {
            // nextPage = Math.min(mItemCount - 1, mSelectedPosition + getChildCount() - 1);
            nextPage = Math.min(mItemCount - 1, selectedIndex + (visibleCells * numberOfPages));
            down = true;
        }
